...
* @return token string
*/
 public static String getToken(String remote_filename, int ts, String secret_key) throws UnsupportedEncodingException, NoSuchAlgorithmException, FdfsException
 {
 	byte[] bsFilename = remote_filename.getBytes(FdfsClientConfig.g_charset);
 	byte[] bsKey = secret_key.getBytes(FdfsClientConfig.g_charset);
 	byte[] bsTimestamp = (new Integer(ts)).toString().getBytes(FdfsClientConfig.g_charset);
