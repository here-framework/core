...
*/
	public TrackerGroup(FdfsClientConfig clientConfig)
	{
		
		
		this.clientConfig=clientConfig;
		
		this.initTrackerServers();
	...
	private void initTrackerServers(){
		TrackerGroup trackerGroup=this.clientConfig.getG_tracker_group();
		this.tracker_servers=trackerGroup.tracker_servers;
	}
...
//		sock.setSoTimeout(clientConfig.g_network_timeout);
//		sock.connect(this.tracker_servers[serverIndex], clientConfig.g_connect_timeout);
//		return new TrackerServer(sock, this.tracker_servers[serverIndex],clientConfig);
		
		
		InetSocketAddress addr=this.tracker_servers[serverIndex];
		Socket sock=clientConfig.getSocket(addr);
...
*/
	...
	public Object clone()
	{
		...
		return new TrackerGroup(clientConfig);
	}
}
