...
*/
	...
	{
		this.tracker_group = fdfsClientConfig.getG_tracker_group();
	}
...
*/
	...
	public StorageServer getStoreStorage(TrackerServer trackerServer, String groupName) throws IOException
	{
		...
		try
		{
			...
			if (groupName != null && groupName.length() > 0)
			{
				...
				bs = groupName.getBytes(fdfsClientConfig.getG_charset());
			...
			return new StorageServer(ip_addr, port, store_path,trackerServer.clientConfig);
		}
	...
	*/
	public StorageServer[] getStoreStorages(TrackerServer trackerServer, String groupName) throws IOException
	{
		...
		try
		{
		    ...
		    results[i] = new StorageServer(ip_addr, port, store_path,trackerServer.clientConfig);
	    }
	...
	*/
			...
			String groupName, String filename) throws IOException
	{
		...
		{
			return new StorageServer(servers[0].getIpAddr(), servers[0].getPort(), 0,trackerServer.clientConfig);
		}
	}
	...
	*/
			...
			byte cmd, String groupName, String filename) throws IOException
	{
		...
		try
		{
			bs = groupName.getBytes(fdfsClientConfig.getG_charset());
			bGroupName = new byte[ProtoCommon.FDFS_GROUP_NAME_MAX_LEN];
			bFileName = filename.getBytes(fdfsClientConfig.getG_charset());
	...
	*/
	public StructGroupStat[] listGroups(TrackerServer trackerServer) throws IOException
	{
		...
		try
		{
			...
			return decoder.decode(pkgInfo.body, StructGroupStat.class, StructGroupStat.getFieldsTotalSize(),trackerServer.clientConfig);
		}
		catch(IOException ex)
		{
			if (!bNewConnection)
	...
	*/
			...
			String groupName, String storageIpAddr) throws IOException
	{
		...
		try
		{
			...
			{
				bIpAddr = storageIpAddr.getBytes(fdfsClientConfig.getG_charset());
			...
			return decoder.decode(pkgInfo.body, StructStorageStat.class, StructStorageStat.getFieldsTotalSize(),trackerServer.clientConfig);
		}
		catch(IOException ex)
		{
			if (!bNewConnection)
	...
	*/
			...
			String groupName, String storageIpAddr) throws IOException
	{
		...
		int ipAddrLen;
		byte[] bIpAddr = storageIpAddr.getBytes(fdfsClientConfig.getG_charset());
		if (bIpAddr.length < ProtoCommon.FDFS_IPADDR_SIZE)
		{
			ipAddrLen = bIpAddr.length;
		}
	...
	*/
	...
	{
		return this.deleteStorage(fdfsClientConfig.getG_tracker_group(), groupName, storageIpAddr);
	}
	...
	*/
	...
	public void setFdfsClientConfig(FdfsClientConfig fdfsClientConfig) {
