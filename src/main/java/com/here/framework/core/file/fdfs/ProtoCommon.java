...
*/
	public static void closeSocket1(Socket sock) throws IOException
	{
		//byte[] header;
		//header = packHeader(FDFS_PROTO_CMD_QUIT, 0, (byte)0);
		//sock.getOutputStream().write(header);
		
		//SocketPool.releaseSocket(sock);
		//sock.close();
	}
	public static void destroySocket(Socket sock) throws IOException
	{
		byte[] header;
		header = packHeader(FDFS_PROTO_CMD_QUIT, 0, (byte)0);
		sock.getOutputStream().write(header);
...
* @return token string
*/
 public static String getToken(String remote_filename, int ts, String secret_key,FdfsClientConfig clientConfig) throws UnsupportedEncodingException, NoSuchAlgorithmException, FdfsException
 {
 	byte[] bsFilename = remote_filename.getBytes(clientConfig.g_charset);
 	byte[] bsKey = secret_key.getBytes(clientConfig.g_charset);
 	byte[] bsTimestamp = (new Integer(ts)).toString().getBytes(clientConfig.g_charset);
...
*/
 public static String genSlaveFilename(String master_filename, 
                String prefix_name, String ext_name) throws FdfsException
 {
    ...
    {
            throw new FdfsException("master filename \"" + master_filename + "\" is invalid");
    }
    ...
    {
        throw new FdfsException("prefix_name \"" + prefix_name + "\" is invalid");
    }
